...
SOUND_DIR="/usr/local/share/asterisk/sounds/custom/asl3-node-list"
...
SOUND_BASE = Path(
    os.environ.get(
        ...
        "/usr/local/share/asterisk/sounds/custom/asl3-node-list/node-list",
    )
)
...
def read_settings(path: Path) -> tuple[configparser.ConfigParser, set[str], str]:
...
def run_tts(node: str, message: str, voice: str, output_base: Path) -> Path:
    ...
    try:
        ...
        temporary_ul.chmod(0o644)
        os.chown(temporary_ul, asterisk.pw_uid, asterisk.pw_gid)
        os.replace(temporary_ul, destination)
        return destination
    finally:
...
def main() -> int:
    ...
    try:
        ...
        cfg, excluded, voice = read_settings(CONFIG_FILE)
        ...
        require_root()
        output = run_tts(nodes[0], message, voice, SOUND_BASE)
...
for path in files:
    ...
    for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        if key == "functions" and value:
            function_stanzas.add(value)
...
for path in files:
    ...
    for lineno, line in enumerate(path.read_text(encoding="utf-8", errors="replace").splitlines(), 1):
        ...
        if section_match:
            current = section_match.group(1).strip()
            continue
        if current not in function_stanzas:
            continue
        assignment_match = assignment_re.match(line)
        if assignment_match and assignment_match.group(1).strip().upper() == code.upper():
            conflicts.append(f"{path}:{lineno}: [{current}] {line.strip()}")
...
for stanza in sorted(function_stanzas):
    print(stanza)
PY_FUNCTION_SCAN
} 2>"$ERROR_FILE")"
STATUS=$?
...
if [[ $STATUS -ne 0 ]]; then
    ...
    if [[ $STATUS -eq 24 ]]; then
        die "DTMF code *$DTMF_CODE is already assigned. Rerun with another code, such as --dtmf 998."
    fi
...
EOF
    while IFS= read -r stanza; do
        [[ -n "$stanza" ]] || continue
        printf '[%s](+)\n' "$stanza"
        printf '%s = localplay,custom/asl3-node-list/node-list\n\n' "$DTMF_CODE"
...
EOF
    ...
    python3 - "$LOCAL_CONFIG" "$VOICE" <<'PY_UPDATE_CONFIG'
import configparser
import sys
from pathlib import Path
